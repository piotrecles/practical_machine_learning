...
A number of the variables are deemed factors when they really shouldn't be (for example kurtosis_roll_belt).  Let's first consider all of the variables that don't look numeric, which for our purposes means the variable is deemed a factor and has fewer than 30 levels.  
...
medians<-sapply(X=1:69,FUN=function(X){summary(training[,X])[3]})
...
and we combine the above with our tree in the following prediction function:
